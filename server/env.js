...
const supportedDBClients = ['pg', 'pg-native', 'sqlite3', 'better-sqlite3', 'mysql', 'mysql2'];
...
if (['--production', '--prod'].some((arg) => process.argv.includes(arg))) {
	process.env.NODE_ENV = 'production';
}

const spec = {
	PORT: num({ default: 3000 }),
	SITE_NAME: str({ example: 'KuaLat', default: 'KuaLat' }),
	DEFAULT_DOMAIN: str({ example: 'kua.lat', default: 'localhost:3000' }),
	...
	MAIL_FROM: str({ default: '', example: '' }),
	MAIL_PASSWORD: str({ default: '' }),
	G_AUTH_ENABLED: bool({ default: false }),
	G_AUTH_CLIENT_ID: str({ default: '' }),
	G_AUTH_CLIENT_SECRET: str({ default: '' }),
	G_AUTH_PROJECT_ID: str({ default: '' }),
...
export const env = cleanEnv(process.env, spec);
