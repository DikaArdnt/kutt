...
export async function up(knex) {
    ...
    if (Number.isNaN(count_number) || count_number === 0)
        return;
    if (count_number < 1_000_000) {
        ...
        await Promise.all(new Array(loops).fill(null).map((_, i) => {
            return knex("visits")
