...
export async function add(params, user) {
	const data = {
		email: params.email,
		password: params.password,
		...(params.role && { role: params.role }),
		...(params.auth_via && { auth_via: params.auth_via }),
		...(params.verified !== undefined && { verified: params.verified }),
		verification_token: randomUUID(),
		verification_expires: dateToUTC(addMinutes(new Date(), 60)),
	};

	if (user) {
		await knex('users')
			.where('id', user.id)
			.update({ ...data, updated_at: dateToUTC(new Date()) });
	} else {
...
export async function update(match, update, methods) {
	const { user, updated_user } = await knex.transaction(async function (trx) {
		...
		await updateQuery.update({ ...update, updated_at: dateToUTC(new Date()) });
...
export async function getAdmin(match, params) {
	...
	if (params?.search) {
		...
		if (Number.isNaN(id)) {
			query.andWhereRaw('LOWER(users.email) LIKE LOWER(?)', ['%' + params.search + '%']);
		} else {
	...
	query.leftJoin(
		knex('domains')
			.select(
				'user_id',
				knex.isMySQL ? knex.raw("group_concat(address SEPARATOR ', ') AS domains") : knex.raw("string_agg(address, ', ') AS domains")
			)
			.groupBy('user_id')
			.as('d'),
		'users.id',
	...
	query.leftJoin(knex('links').select('user_id').count('* as links_count').groupBy('user_id').as('l'), 'users.id', 'l.user_id');

	return query;
}

export async function totalAdmin(match, params) {
	const query = knex('users').count('* as count').fromRaw('users').where(normalizeMatch(match));
	...
	if (params?.links !== undefined) {
		...
		query.leftJoin(knex('links').select('user_id').count('* as links').groupBy('user_id').as('l'), 'users.id', 'l.user_id');
	}
