...
export async function getAdmin(match, params) {
	...
	if (params?.search) {
		query.andWhereRaw("concat_ws(' ', domains.address, domains.homepage) LIKE ?", ['%' + params.search + '%']);
	}
	...
	query.leftJoin(knex('links').select('domain_id').count('* as links_count').groupBy('domain_id').as('l'), 'domains.id', 'l.domain_id');
