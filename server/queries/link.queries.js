...
export const selectable_admin = [...selectable, 'users.email as email'];
...
export async function total(match, params) {
	...
	if (params?.search) {
		query.andWhereRaw("concat_ws(' ', description, links.address, target, domains.address) LIKE ?", ['%' + params.search + '%']);
	}
...
export async function totalAdmin(match, params) {
	...
	if (params?.search) {
		query.andWhereRaw("concat_ws(' ', description, links.address, target) LIKE ?", ['%' + params.search + '%']);
	}

	if (params?.domain) {
		query.andWhereRaw('LOWER(domains.address) LIKE LOWER(?)', ['%' + params.domain + '%']);
	}
...
export async function get(match, params) {
	const query = knex('links')
		.select(...selectable)
...
export async function getAdmin(match, params) {
	...
	query.orderBy('links.id', 'desc').offset(params.skip).limit(params.limit);
...
export async function create(params) {
	...
	let [link] = await knex('links').insert(
		{
...
export async function remove(match) {
...
export async function batchRemove(match) {
	const query = knex('links');
...
export async function update(match, update) {
	if (update.password) {
		const salt = await bcrypt.genSalt(12);
	...
	await knex('links')
		.where(match)
		.update({ ...update, updated_at: dateToUTC(new Date()) });

	const updated_links = await knex('links').select(selectable).where(normalizeMatch(match)).leftJoin('domains', 'links.domain_id', 'domains.id');
...
export function incrementVisit(match) {
